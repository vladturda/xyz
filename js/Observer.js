var Observer = function( position = new Point(), target = new Vector(), focal_length = 0 ) {
  ...
  this.moveRelative = function( direction = new Vector() ) {
    ...
    var y_axis = this.projection_matrix.rows[1];
  ...
  this.rotateAxis = function( rotation = new Vector() ) {
    ...
    var target_vector = this.target.matrix().subtract(this.position.matrix());
    ...
    if(rotation.x) {
      ...
      target_vector = rotation_matrix.multiply(target_vector);
    }
    ...
    this.setTarget(target_vector.add(this.position.matrix()).vector());
  };
  ...
  this.project = function( point_matrix = new Matrix() ) {
    ...
    var z_scale = -this.focal_length / projection_point.rows[2][0];
